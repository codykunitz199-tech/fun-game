...
const world = {
  ...
  damagePopups: [],
  prompt: null,
  gameOver: false // per-world toggle; for per-player gameOver, track on player
};
...
function makeDefaultPlayer(id) {
  return {
    ...
    drones: [], droneMax: 10, droneRadius: 8, droneSpeed: 4, droneDamage: 6,
    ...
    trapLayer: false, traps: [], trapMax: 10, trapBaseDamage: 10,
...
/* ===== Targeting utilities (consider all players) ===== */
...
function trapClusterExplode(player, t) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    player.bullets.push({
      x: t.x, y: t.y, dx, dy,
      r: 3, source: "player", ownerId: player.id, dmg: shardDamage,
...
function spawnFragments(player, x, y, baseDmg) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    player.bullets.push({
      x, y, dx, dy,
      r: 3, source: "player", ownerId: player.id, dmg: Math.floor(baseDmg * 0.4),
      spawnTime: PERF.now(), lifeTime: 500, explosive: false, pierce: 0, shockwave: false, hitCooldown: {}
    });
  }
}
...
function switchPath(player, newPath) {
  ...
  if (newPath === "multi") {
    ...
    player.trapBaseDamage = 10; player.trapBaseSize = 12; player.trapBaseCooldown = 2000; player.nextTrapTime = 0;
  }
}
...
function updatePlayerDrones() {
  for (const owner of world.players.values()) {
    ...
    for (let di = owner.drones.length - 1; di >= 0; di--) {
      ...
      // PvP: drones bump into other players (optional)
      for (const other of world.players.values()) {
...
/* ===== Entity pushing (no self-push to owner) ===== */
function resolveEntityCollisions() {
  // Build entity list across all players
  const entities = [];
  ...
  for (let i = 0; i < entities.length; i++) {
    for (let j = i + 1; j < entities.length; j++) {
      ...
      if (dist > 0 && dist < minDist) {
        ...
        // No self-push for a player's own drones/traps
...
function buildSnapshotForClient() {
  return {
    ...
    // Per-player prompts handled client-side: send for everyone; client shows only for self
    prompt: null,
    gameOver: false // global off; clients show per-player death
  };
}
...
function tick() {
  ...
  for (const p of world.players.values()) checkLevelMilestones(p);
...
io.on("connection", socket => {
  ...
  socket.on("respawn", () => {
    ...
    p.hp = 100; p.maxHp = 100; p.xp = 0; p.level = 1; p.dead = false;
