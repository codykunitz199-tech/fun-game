...
function makeDefaultPlayer(id) {
  return {
    ...
    regenPerSec: 0, // HP/sec (10 for dread cannon)

    // Drone dreadnought spawners
    dreadDroneSpawnerA: 0, dreadDroneSpawnerB: 0, // next spawn times
    dreadDroneFireCooldown: 0, // per drone fire timer baseline
      ...
      if (world.boss.hp > 0) {
        ...
        if (PERF.now() >= immuneUntilB) {
          ...
          if (distBoss < world.boss.r + b.r) {
            ...
            // Boss death handling: award XP and respawn
            if (world.boss.hp <= 0) {
...
function spawnDreadDrone(owner, fromAngle) {
  // Super drone: 30 contact dmg, disappears on contact; has a 10-dmg shot every 3s
...
function checkLevelMilestones(player) {
  if (player.dead) return;
  const threshold = player.level * 50; // keep original pacing
...
function playerFireTick(player) {
  ...
  if (!player.dead && player.mainGunEnabled && t >= player.nextFireTime) {
    ...
    // Alternating Fire override to 100ms only if AlternatingFire is active and NOT Wall of Lead
...
function tick() {
  for (const p of world.players.values()) {
    ...
    // Regen for dreadnought cannon: 10 hp/sec
